...
function _resolve_reports_dir(path::AbstractString)::String
    ...
    if isdir(exports_root)
        ...
        for entry in readdir(exports_root; join = true)
            if isdir(entry) &&
               all(isfile(joinpath(entry, f)) for f in _REQUIRED_REPORT_FILES)
...
_as_f32(x) = x === nothing ? 0.0f0 : Float32(x)
_as_str(x) = x === nothing ? "" : String(x)
_shape_str(s) =
    s === nothing ? "" : s isa AbstractString ? String(s) : join(string.(s), "x")
...
function load_report_bundle(path::AbstractString)::XAIReportBundle
    isempty(path) &&
        throw(ArgumentError("load_report_bundle: path is required (no synthetic fallback)"))
    isdir(path) || throw(ArgumentError("load_report_bundle: not a directory: $path"))

    dir = _resolve_reports_dir(path)
    missing_files = [f for f in _REQUIRED_REPORT_FILES if !isfile(joinpath(dir, f))]
    isempty(missing_files) || throw(
        ArgumentError(
