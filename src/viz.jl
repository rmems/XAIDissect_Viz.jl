...
function launch_atmosphere(bundle::XAIReportBundle; backend::ComputeBackend = CPUBackend())
    try
        ...
        error(
            "launch_atmosphere requires GLMakie, GraphMakie, Graphs, and Observables, " *
...
function _launch_atmosphere(bundle::XAIReportBundle; backend::ComputeBackend = CPUBackend())
    ...
    try
        cache = build_frame_cache(
            bundle;
            backend = CPUBackend(),
            n_tokens = n_tokens_default,
            seed = cache_seed,
        )
    catch err
        @warn "build_frame_cache failed; falling back to direct simulate_router_frame path" error =
            err
        cache = nothing
    end
    ...
    current_frame =
        Observable(simulate_router_frame(bundle, 1, 0; backend = backend, seed = seed[]))
    ...
    topk_buf =
        cache === nothing ? Matrix{Int32}(undef, 0, 0) :
    ...
    if backend isa CUDABackend && cache !== nothing && cuda_avail
        try
            ...
            @warn "CUDA activity buffer init failed; per-tick activity will run on CPU" error =
                err
        end
    end
    ...
    function _fill_topk_buf!(t::Int)
        ...
        @inbounds for j = 1:(cache.top_k), b = 1:n_blocks
    ...
    function _step_activity!(t::Int)
        ...
        if use_cuda_activity[]
            try
                ...
                @warn "CUDA activity tick failed; switching to CPU activity for this session" error =
                    err
    ...
    edge_to = Int[]
    push!(edge_from, 1)
    push!(edge_to, 2)                    # input -> router
    for e = 1:n_experts
        push!(edge_from, 2)
        push!(edge_to, 2 + e)            # router -> expert
        push!(edge_from, 2 + e)
        push!(edge_to, 2 + n_experts + 1)  # expert -> out
    end
    g = SimpleDiGraph(length(node_pos))
    for (f, t) in zip(edge_from, edge_to)
        add_edge!(g, f, t)
    end
        ...
        for i = 1:length(node_colors[])
            ...
            elseif i > 2 && i <= 2 + n_experts
                ...
                if eid in top2
                    ...
                    node_sizes[][i] =
                        Float32(32 + 6 * sin(2π * (frame.token_idx % 30) / 30))  # pulse
                else
    ...
    onany(selected_block, current_frame) do blk, frame
        if frame.block != blk
            new_frame = simulate_router_frame(
                bundle,
                blk,
                token_idx[];
                backend = backend,
    ...
    Label(
        ...
        lift(
            f ->
                "Logits (first 4): $(round.(f.logits[1:min(4, length(f.logits))]; digits=3)) …",
            current_frame,
        ),
    )
    ...
    Label(
        inspector[6, 1],
        lift(f -> "Entropy: $(round(f.entropy; digits=4))", current_frame),
    )
    Label(
        inspector[7, 1],
        lift(selected_block) do b
    ...
    Label(
        inspector[8, 1],
        "Provenance: $(bundle.provenance) — simulated router dynamics on real metadata",
    )
    Label(
        inspector[9, 1],
        "Click heatmap row or use slider to change block",
        fontsize = 10,
        color = :gray,
    )
    ...
    perf_label = Label(
        ...
        lift(token_idx, selected_block, fps_obs) do t, b, f
            ...
            cuda_str =
                backend isa CUDABackend ? (cuda_avail ? "true" : "false") :
                "n/a (CPUBackend)"
            cache_str =
                cache === nothing ? "off (fallback)" :
    ...
    play_btn =
        Button(timeline[1, 2], label = lift(p -> p ? "⏸ Pause" : "▶ Play", is_playing))
    on(play_btn.clicks) do _
        is_playing[] = !is_playing[]
        if !is_playing[]
            current_frame[] = simulate_router_frame(
                bundle,
                selected_block[],
                token_idx[];
                backend = backend,
                ...
                try
                    ...
                    while is_playing[]
                        ...
                        if dt > 0
                            inst = 1.0 / dt
                            fps_obs[] =
                                fps_obs[] == 0 ? inst : 0.6 * fps_obs[] + 0.4 * inst
                        end
                        sleep(0.08)
                    end
                catch e
