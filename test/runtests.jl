...
@testset "CUDABackend: rejects host Matrix when CUDA functional (gated)" begin
    if _cuda_functional()
        A = zeros(Float32, 4, 8)
        T = rand(Int32(1):Int32(8), 4, 2)
        @test_throws ArgumentError update_activity_field!(XAIDissectViz.CUDABackend(), A, T)
    else
        @test true
    end
end
...
@testset "RouterFrameCache: dimensions and exact contracts" begin
    ...
    @test cache.top_k == 2
    @test cache.n_tokens == 20  # max token index (cache stores 0..n_tokens)
...
@testset "build_frame_cache: ArgumentError when metadata missing keys" begin
    bad = XAIReportBundle(
        Dict{String,Any}("d_model" => 6144, "n_experts" => 8),
        RouterRecord[],
        ExpertRecord[],
        TensorMetricRecord[],
        SAAQReadinessRecord[],
    ...
    @test_throws ArgumentError build_frame_cache(bad; n_tokens=2)

    bad2 = XAIReportBundle(
        Dict{String,Any}("d_model" => 6144, "n_blocks" => 64),
        RouterRecord[],
        ExpertRecord[],
        TensorMetricRecord[],
        SAAQReadinessRecord[],
    ...
    @test_throws ArgumentError build_frame_cache(bad2; n_tokens=2)
end
