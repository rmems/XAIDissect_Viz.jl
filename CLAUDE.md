...
**`RouterFrameCache` (`cache.jl`)**: Precomputes `simulate_router_topk_batch` for token indices `0:n_tokens` at launch (`n_tokens` defaults to 300, giving 301 slots — `cache.n_tokens == n_tokens + 1`), so the global heatmap only steps the cheap `update_activity_field!` per tick during playback instead of re-simulating a full forward pass. The per-block inspector panel is not fully covered by the cache: `_launch_atmosphere`'s play loop (`src/viz.jl`) still calls the heavier `simulate_router_frame` every 5th tick (and on slider/play-button interaction) to refresh it. `activity_matrix_for_token` reconstructs the activity field at a given token by replaying cached top-k matrices from zero — deterministic given `(seed, decay, boost)`. Token indices are 0-based (matching the UI slider); `_token_pos` converts to Julia's 1-based storage.

**Viz layout (`viz.jl`)**: `launch_atmosphere` renders four regions — (A) global 64×8 heatmap + risk strip, (B) selected-block routing graph, (C) inspector panel (logits/probs/entropy/SAAQ/provenance), (D) timeline (token slider, play/pause, seed) — driven by independent local `Observable`s inside `_launch_atmosphere` (`selected_block`, `token_idx`, `activity`, `is_playing`, `seed`, `current_frame`). Note `AtmosphereState` (`types.jl`) mirrors those same fields and is exported, but nothing in `src/` currently constructs one — treat it as an aspirational/unused struct, not the actual state container, until something wires it in.
...
- `Aqua.test_all` (in `test/runtests.jl`) intentionally disables two checks — this is expected, not a bug to fix. `stale_deps = false` because the lazily-loaded packages (CUDA, GLMakie, GraphMakie, Graphs, Observables, Makie) aren't imported at module init, so Aqua's static analysis would otherwise flag them as unused. `piracies = false` per that file's own comment ("CUDA @eval is intentional augmentation, not piracy") — note the current CUDA dispatch methods (`router_logits(::CUDABackend, ...)`, etc.) extend XAIDissectViz's own functions with XAIDissectViz's own types, so they wouldn't trip Aqua's piracy check on their own merits either way; don't take this exemption as license to add a method that pirates an external function/type.
